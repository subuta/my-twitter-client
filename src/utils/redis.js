...
const retryStrategy = (options) => {
  ...
  if (options.attempt >= RETRY_ATTEMPT) {
    ...
    console.error(`[redisSubscribeClient] Cannot reconnect to redis while ${RETRY_ATTEMPT} times. TOTAL_RETRY_TIME = ${options.total_retry_time / 1000}s`)
    console.error(`[redisSubscribeClient] App will exit.`)
...
redisSubscribeClient.on('message', (channel, data) => {
  ...
  console.log(`[redisSubscribeClient] Got message at ${channel}`)
  if (!listeners) return
...
redisSubscribeClient.on('reconnecting', ({delay, attempt}) => {
  console.log(`[redisSubscribeClient] Try to reconnecting ${attempt} times, with delay = ${delay / 1000}s.`)
})

redisSubscribeClient.on('connect', () => {
  console.log('[redisSubscribeClient] Successfully connected to redis.')
})

redisSubscribeClient.on('ready', () => {
  console.log('[redisSubscribeClient] Connection with redis become ready state.')
})

redisSubscribeClient.on('end', () => {
  console.log('[redisSubscribeClient] Connection with redis ended successfully.')
})

redisSubscribeClient.on('error', (err) => {
  console.error(`[redisSubscribeClient] Got error with redis connection, err = ${err.message}.`)
  console.error(err)
})
...
export const subscribe = (channel, cb) => {
  ...
  console.log(`[redisSubscribeClient] Subscribed to '${channel}', listeners = ${channelListeners[channel].length}.`)
}
...
export const unsubscribe = (channel, cb) => {
  ...
  console.log(`[redisSubscribeClient] UnSubscribed from '${channel}', listeners = ${channelListeners[channel].length}.`)
}
